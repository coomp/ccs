...
func TestWeakDecodeJson(t *testing.T) {
	type args struct {
		input  interface{}
		output interface{}
	}
	...
	tests := []struct {
		...
		{"test", args{map[string]interface{}{"name": "zhenheng,sofent,lol", "age": 18}, &user{}}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := WeakDecodeJson(tt.args.input, tt.args.output); (err != nil) != tt.wantErr {
				t.Errorf("WeakDecodeJson() error = %v, wantErr %v", err, tt.wantErr)
			} else {
				t.Log("output is ", tt.args.output)
			}
		})
	}
}
